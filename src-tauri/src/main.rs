...
use MiraiCL_lib::{ext_mod, tauri_generate_context};
...
fn main() -> Result<Infallible, Box<dyn Error>> {
    ...
    // 👉 Equivalent to `python -m MiraiCL`,
    // i.e, run the `src-tauri/python/MiraiCL/__main__.py`
    let py_script = PythonScript::Module("MiraiCL".into());
