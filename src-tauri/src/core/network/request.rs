...
use anyhow::Result;
use reqwest::{Client, Method, Proxy, tls::Version,Body,header::HeaderName,Error};
use url::Url;
...
fn start_up() -> Result<()>{
    ...
    *client = Some(Client::builder()
        .brotli(true)
        .zstd(true)
        .gzip(true)
        .deflate(true)
        .min_tls_version(Version::TLS_1_2)
        .user_agent("MiraiCL").build()?);
    Ok(())
}

fn setup_proxy(address:String,port:u16,username:Option<String>,password:Option<String>) -> Result<()>{
    let mut client = CLIENT.write().unwrap();
    *client = None;
    let mut proxy = match Url::parse(address.as_str())?.scheme(){
            "http" => {
...
发送单次网络请求并等待返回
*/
pub async fn make_request_once(url:&str,method:Method,headers:Option<HashMap<HeaderName,String>>,data:Option<Body>,timeout:Option<u64>,log:Option<bool>) -> Result<Response>{
    let mut client = CLIENT.read()?.unwrap();
